...
const ScenesPage = (() => {
  ...
  function render() {
    ...
    el.innerHTML = Data.scenes.map(s => `
      <div class="room-card scene-card-full${s.active?' is-on':''}" data-scene="${s.id}" style="padding:var(--sp-4)">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:var(--sp-3)">
          <div style="width:44px;height:44px;border-radius:var(--r-md);background:${s.color}22;display:flex;align-items:center;justify-content:color:${s.color}">
            <div style="margin:auto;color:${s.color}">${sceneIcon(s.icon,22)}</div>
          </div>
    ...
    el.querySelectorAll('.btn-activate-scene').forEach(b => {
      b.addEventListener('click', e => {
        e.stopPropagation();
        Data.activateScene(b.dataset.scene);
    ...
    el.querySelectorAll('.btn-del-scene').forEach(b => {
      b.addEventListener('click', e => {
        ...
        Data.deleteScene(b.dataset.scene);
        render();
      });
    });
  ...
  function sceneFormHtml(s={}) {
    return `
      ...
      <div class="input-wrap"><label>Ícone</label>${emojiPicker(s.emoji||'💡', ()=>{})}
      </div>
  ...
  function openAddScene() {
    ...
    bindSlider(
      document.getElementById('m-sc-bri-track'), document.getElementById('m-sc-bri-fill'),
      document.getElementById('m-sc-bri-thumb'), document.getElementById('m-sc-bri-lbl'),
      80, v => chosenBri = v
    );

    document.getElementById('m-save-scene').addEventListener('click', () => {
      const name = document.getElementById('m-sc-name').value.trim();
      if (!name) return toast('Informe o nome da cena');
      Data.addScene({ name, desc: document.getElementById('m-sc-desc').value.trim(), icon: 'scene', emoji: chosenEmoji, brightness: chosenBri, temp: chosenTemp, color: chosenColor });
      Modal.close();
    });
  }
  ...
  function openEditScene(sceneId) {
    ...
    bindSlider(
      document.getElementById('m-sc-bri-track'), document.getElementById('m-sc-bri-fill'),
      document.getElementById('m-sc-bri-thumb'), document.getElementById('m-sc-bri-lbl'),
      s.brightness, v => chosenBri = v
    );

    document.getElementById('m-save-scene').addEventListener('click', () => {
      const name = document.getElementById('m-sc-name').value.trim();
      if (!name) return toast('Informe o nome');
      Data.editScene(sceneId, { name, desc: document.getElementById('m-sc-desc').value.trim(), emoji: chosenEmoji, brightness: chosenBri, temp: chosenTemp, color: chosenColor });
      Modal.close();
    });
  }
